...
1.  **Set Secret**: Register `JWT_SECRET` as an encrypted secret (do NOT write it in `wrangler.toml`).
    ```bash
    npx wrangler secret put JWT_SECRET
    # Paste a long random string, e.g. generated by:
    # node -e "console.log(require('crypto').randomBytes(32).toString('base64url'))"
    ```
    For the Preview environment, run `npx wrangler secret put JWT_SECRET --env preview` as well.
    For local development, put a value in `.dev.vars` (gitignored):
    ```
